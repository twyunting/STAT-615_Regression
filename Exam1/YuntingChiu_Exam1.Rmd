...
```
- According to the `outlierTest` table, the Bonferroni p-value for the largest (absolute) residual is not statistically significant, which means there are no outliers.
...
(g) Compute a 90% prediction interval for the response Y0 if the corresponding independent variable is $X_0$ = 35

References:
- http://www.r-tutor.com/elementary-statistics/numerical-measures/correlation-coefficient#:~:text=The%20correlation%20coefficient%20of%20two,the%20two%20are%20linearly%20related.
