...
```
## 2.2
- Ho: β1 = 0 (have NOT a linear association between ACT score (X) and GPA at the end of the freshman year (Y)).
- Ha: β1 ≠ 0 (have a linear association between ACT score (X) and GPA at the end of the freshman year (Y)).
...
If ACT score is 28, wehave 95% certain contains the population mean of freshman GPA is 3.061384 to 3.341033.
```{r}
predict(reg, data.frame(X = 28), interval = "confidence", level = 0.95)
```

## 2.4
For the individual response (actual response) $Y$ - prediction intervals
