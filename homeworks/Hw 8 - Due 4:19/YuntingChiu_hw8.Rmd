...
- Advantage of small $\alpha$ to-enter value: 
...
- Because $R^2$ is not a fair measurement. As the number of parameters increases, so does the $R2$.
```{r}
...
- We can also see if there is a linear relationship between the independent and dependent variables in scatterplot. According to the plot, `Y` and `X1` does have a linear relationship, but `X2` and `Y` does not. Also, the model may have a multicollinearity problem as `X1` and `X2` appear to have a linear relationship so consider keeping `lm(Y ~ X1)` to run a linear model.
